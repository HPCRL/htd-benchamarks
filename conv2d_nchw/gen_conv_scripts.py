sizesConv2d_nchw = [
    [1, 112, 112, 128, 128, 3, 3, 1, 1],   
    [1, 14, 14, 512, 512, 3, 3, 1, 1],
    [1, 224, 224, 64, 3, 3, 3, 1, 1],
    [1, 224, 224, 64, 3, 7, 7, 2, 3],
    [1, 28, 28, 512, 512, 3, 3, 1, 1],
    [1, 56, 56, 256, 256, 3, 3, 1, 1],
]
sizesVGG = [
...
for item in sizesVGG + sizesResnet + sizesYolo:
    ...
    with open("../../../test/conv2d_nchw/conv2d.mlir", "rt") as fin:
        with open(f"../../../test/conv2d_nchw/input_mlir/conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}.mlir", "wt") as fout:
            for line in fin:
    ...
    with open("../../../test/conv2d_nchw/test_conv2d.sh", "rt") as fin:
        with open(f"../../../test/conv2d_nchw/bash_scripts/conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80.sh", "wt") as fout:
            for line in fin:
                fout.write(line.replace('${N}', str(N)).replace("${H}", str(H + 2 * pad)).replace("${W}", str(W + 2 * pad)).replace("${CO}", str(CO)).replace("${CI}", str(CI)).replace("${KH}", str(KW)).replace("${KW}", str(KW)).replace("${ST}", str(stride)).replace("${OH}", str(output_height)).replace("${OW}", str(output_width)))