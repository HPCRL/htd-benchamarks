...
if mode != "timing":
    with open(f"all_conv2d_nchw_{mode}.sh", 'w') as f:
    ...
    with open(f"all_conv2d_nchw_{mode}.sh", 'w') as f:
        for item in sizesVGG + sizesResnet + sizesYolo:
            ...
            f.write(f"echo \"Running conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80\"\n")
            f.write(f"cd /uufs/chpc.utah.edu/common/home/u1419116/projects/hlt/codegen/tvm/src/conv2d_nchw && bash {mode}.slurm {' '.join(map(str, item))} &> ../../../results/conv2d_nchw/tvm/conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80.txt\n")
            f.write(f"cd /uufs/chpc.utah.edu/common/home/u1419116/projects/hlt/codegen && cp auto_tuner_cpp/conv2d_nchw/conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_strides_{stride}_padding_{pad}_sm_80.cpp main.cpp && cd build && rm -rf * && cmake .. && make && ./codegen && cd ../test/conv2d_nchw/bash_scripts && bash conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80.sh &> ../../../results/conv2d_nchw/autotuner/conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80.txt && cd ../../../tvm/src/conv2d_nchw/\n")
            f.write(f"cd /uufs/chpc.utah.edu/common/home/u1419116/projects/hlt/codegen && cp tvm/ansor_schedule/conv2d_nchw/conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_strides_{stride}_padding_{pad}_sm_80.cpp main.cpp && cd build && rm -rf * && cmake .. && make && ./codegen && cd ../test/conv2d_nchw/bash_scripts && bash conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80.sh &> ../../../results/conv2d_nchw/same_schedule/conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80.txt && cd ../../../tvm/src/conv2d_nchw/\n")
