...
with open(f"all_dilated_conv2d_nchw_{mode}.sh", 'w') as f:
    for item in arr:
        ...
        with open("../../../test/dilated_conv2d_nchw/test_dilated_conv2d.sh", "rt") as fin:
            with open(f"../../../test/dilated_conv2d_nchw/bash_scripts/dilated_conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_dilation_{dilation}_sm_80.sh", "wt") as fout:
                # TODO: add dilation
                for line in fin:
                    fout.write(line.replace('${N}', str(N)).replace("${H}", str(H + 2 * pad)).replace("${W}", str(W + 2 * pad)).replace("${CO}", str(CO)).replace("${CI}", str(CI)).replace("${KH}", str(KW)).replace("${KW}", str(KW)).replace("${ST}", str(stride)).replace("${OH}", str(output_height)).replace("${OW}", str(output_width)).replace("${OLD_H}", str(H)).replace("${OLD_W}", str(W)).replace("${PAD}", str(pad)))
