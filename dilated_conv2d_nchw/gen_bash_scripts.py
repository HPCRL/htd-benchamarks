...
with open(f"all_dilated_conv2d_nchw_{mode}.sh", 'w') as f:
    for item in sizesVGG + sizesResnet + sizesYolo:
        ...
        with open("../../../test/dilated_conv2d_nchw/test_dilated_conv2d.sh", "rt") as fin:
            with open(f"../../../test/dilated_conv2d_nchw/bash_scripts/dilated_conv2d_nchw_N{N}_H{H}_W{W}_CO{CO}_CI{CI}_KH{KH}_KW{KW}_ST_{stride}_padding_{pad}_sm_80.sh", "wt") as fout:
                for line in fin:
